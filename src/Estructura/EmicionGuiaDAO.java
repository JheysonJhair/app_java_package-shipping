...
public class EmicionGuiaDAO {
    ...
    public int idFactura(){
        int id = 0;
        String sql = "SELECT MAX(id) FROM tFactura";
        try {
    ...
    //Registrar Facturacion DAO
    public int RegistrarFactura(Facturacion v){
        String sql = "INSERT INTO tFactura (cliente, vendedor, total, fecha) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    //Registrar Detalle DAO
    public int RegistrarDetalle(Detalle Dv){
       String sql = "INSERT INTO tDetalle (id_pro, cantidad, precio, id_Factura) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public List ListarEnvios(){
       ...
       String sql = "SELECT c.id AS id_cli, c.nombre, v.* FROM tCliente c INNER JOIN tFactura v ON c.id = v.cliente";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           rs = ps.executeQuery();
           while (rs.next()) {               
               ...
               fact.setNombre_cli(rs.getString("nombre"));
               fact.setVendedor(rs.getString("vendedor"));
    ...
    public void pdfV(int idventa, int Cliente, double total, String usuario) {
        try {
            ...
            File salida = new File(url + "ventaEg.pdf");
            archivo = new FileOutputStream(salida);
            Document doc = new Document();
            PdfWriter.getInstance(doc, archivo);
            doc.open();
            Image img = Image.getInstance(getClass().getResource("/Img/LogoPrincipalMarvisur.png"));
            //Fecha
            ...
            PdfPTable detalleg = new PdfPTable(3);

            detalleg.setWidthPercentage(100);
            detalleg.getDefaultCell().setBorder(0);
            detalleg.setWidths(columnWidthsCliente);
            detalleg.setHorizontalAlignment(Element.ALIGN_LEFT);

            Paragraph dg = new Paragraph();
            dg.add(Chunk.NEWLINE);
            dg.add("DATOS GENERALES" + "\n\n");
            doc.add(dg);

            PdfPCell dgNom = new PdfPCell(new Phrase("Conductor", negrita));
            PdfPCell dgPlac = new PdfPCell(new Phrase("Placa", negrita));
            PdfPCell dgDestin = new PdfPCell(new Phrase("Destino", negrita));
            
            dgNom.setBorder(Rectangle.NO_BORDER);
            dgPlac.setBorder(Rectangle.NO_BORDER);
            dgDestin.setBorder(Rectangle.NO_BORDER);
            
            detalleg.addCell(dgNom);
            detalleg.addCell(dgPlac);
            detalleg.addCell(dgDestin);

            String detalleGu = "SELECT * FROM tConductor WHERE id = ?";
            try {
                ps = con.prepareStatement(detalleGu);
                ps.setInt(1, Cliente);
                rs = ps.executeQuery();
                if (rs.next()) {
                    detalleg.addCell(rs.getString("nombre"));
                    detalleg.addCell(rs.getString("vehiculo"));
                    detalleg.addCell(rs.getString("direccion") + "\n\n");
                } else {
                    detalleg.addCell("Publico en General");
                    detalleg.addCell("S/N");
                    detalleg.addCell("S/N" + "\n\n");
                }
            ...
            tabla.addCell(c3);
            tabla.addCell(c4);
            String product = "SELECT d.id, d.id_pro,d.id_factura, d.precio, d.cantidad, p.id, p.nombre FROM tDetalle d INNER JOIN tProducto p ON d.id_pro = p.id WHERE d.id_factura = ?";
            try {
